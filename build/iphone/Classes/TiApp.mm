...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kwebview_event_bubblingUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kwebview_event_bubblingUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
