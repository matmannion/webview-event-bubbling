...
static NSString * const kwebview_event_bubblingJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_webview_event_bubblingInjection
{
	...
	[html appendString:kwebview_event_bubblingJavascript];
...
{
	// attempt to make well-formed HTML and inject in our webview_event_bubbling bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _webview_event_bubblingInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to webview_event_bubbling support.",url);
				}
			}
