...
NSString * webview_event_bubbling$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation webview_event_bubblingObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_webview_event_bubbling);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
-(void)injectPatches
{
	// called to inject any webview_event_bubbling patches in JS before a context is loaded... nice for 
...
{
	// create webview_event_bubbling global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_webview_event_bubbling = [[webview_event_bubblingObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_webview_event_bubbling];
	
	NSString *_webview_event_bubblingNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _webview_event_bubblingNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_webview_event_bubbling valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:webview_event_bubbling$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.webview_event_bubbling.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.webview_event_bubbling.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
